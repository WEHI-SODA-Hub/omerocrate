...
class ImageRequest(TaskQueueBase):
    ...
    tag: list[KeyValue] = Field(
        default_factory=list, description="List of tags associated with this image"
    )
...
class DatasetFields(TaskQueueBase):
    ...
    tag: list[KeyValue] = Field(
        default_factory=list, description="List of tags associated with this dataset"
    )
...
class DatasetRequest(DatasetFields):
    ...
    image: list[ImageRequest] = Field(
        default_factory=list, description="List of images in this dataset"
    )
...
class ProjectFields(TaskQueueBase):
    ...
    tag: list[KeyValue] = Field(
        default_factory=list, description="List of tags associated with this project"
    )
...
class ProjectRequest(ProjectFields):
    ...
    dataset: Annotated[
        List[DatasetRequest],
        Field(default_factory=list, description="List of datasets in this project"),
    ]
...
class UploadFields(TaskQueueBase):
    ...
    group: Annotated[str, Field(description="The OMERO group name")]
    import_user: Annotated[
        str,
        Field(
...
State = Annotated[
    Literal["PENDING", "FAILURE", "STARTED", "SUCCESS", "IMPORT-IN-PROGRESS"],
    Field(description="The status of the upload task"),
]
...
ImportStatus = Annotated[
    Optional[Literal["SUCCEEDED", "FAILED"]],
    Field(
