...
def upload(
    crate: Annotated[
        Path, typer.Argument(help="Path to the directory containing the RO-Crate")
    ],
    uploader_path: Annotated[
        str,
        typer.Option(
            "--uploader-path", "-u", help="Module path to the OmeroUploader class"
        ),
    ] = "omerocrate.ApiUploader",
    seg_uploader_path: Annotated[
        str,
        typer.Option(
            "--seg-uploader-path",
            "-s",
            help="Module path to the SegmentationUploader class",
        ),
    ] = "omerocrate.SegmentationUploader",
    seg_upload_dir: Annotated[
        Union[Path, None],
        typer.Option(
            "--seg-upload-dir", "-d", help="Directory to upload segmentations"
        ),
    ] = None,
):
    ...
    if not issubclass(seg_uploader_cls, SegmentationUploader):
        raise typer.BadParameter(
            f"{seg_uploader_path} is not a valid SegmentationUploader class"
        )

    conn = from_env()
    seg_uploader = seg_uploader_cls(conn=conn, upload_directory=seg_upload_dir)
    uploader = uploader_cls(conn=conn, crate=crate, segmentation_uploader=seg_uploader)
    dataset = run(uploader.execute())
    # stderr
    typer.echo(
        f"Uploaded dataset with ID: {dataset.id} and name: {dataset.name}", err=True
    )
    # stdout
